...
`holodeck.fort.galaxy.rip` inside the NOLA phone app, on the **Game Servers** tab.
...
```yaml
  - job_name: cadvisor
    static_configs:
      - targets:
          - holodeck.fort.galaxy.rip:8080   # add holodeck to the existing list
```
...
> `holodeck.fort.galaxy.rip:8080`, or an FQDN all work. If you scrape it by raw
...
```yaml
  - job_name: mysql
    static_configs:
      - targets: ['holodeck.fort.galaxy.rip:9104']
        labels:
...
```js
// Optional friendly names, e.g. "holodeck.fort.galaxy.rip:9104=Holodeck MariaDB"
