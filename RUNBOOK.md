...
1. Check n8n execution history at `http://n8n.example.com/executions`
...
- Prometheus scrape target down → check `http://prometheus.example.com/targets`
...
### NOLA dashboard shows ALL hosts down (but they're up)
The dashboard derives every host's up/down state from `up{job="node"}` in Prometheus.
...
```
Note its restart policy is `unless-stopped`, so a **manual** `docker stop` is honored and
it will NOT auto-restart on its own — it stays down until brought back up. (This caused a
9-day silent outage in June 2026.)

### Prometheus Down alert / watchdog
A dead Prometheus cannot alert on itself, so detection lives in **Grafana** (a separate
container). Alert rule **"Prometheus Down"** (uid `prometheus-down-wd`, group
`Homelab Alerts`, severity `critical`) queries `up{job="prometheus"}` with
**`noDataState=Alerting` and `execErrState=Alerting`** — so when Grafana can't reach the
Prometheus datasource the rule fires a real alert carrying the `severity` label and routes
to Discord + Email. (The older `up==0` "Prometheus Scrape Down" rule uses
`execErrState=Error`, whose synthetic DatasourceError alert lacks `severity` and silently
falls through the notification policy — which is why it never paged.)
...
- For halt.universe: URL is `halt.universe` (not `halt.example.com`)
...
docker compose up -d
docker exec nola-dashboard wget -qO- http://127.0.0.1:3000/api/health   # verify
```
...
2. Add a scrape target to `~/deploy/prometheus/prometheus.yml` on union:
   ```yaml
   - targets: ['<IP>:9100']
     labels:
       instance: <hostname>
...
4. Add the hostname to `LINUX_HOSTS` (or `PVE_HOSTS`) in `~/nola/dashboard/.env` on union
5. Restart dashboard: `cd ~/nola/dashboard && docker compose up -d`

### Container stats (cAdvisor) — hosts and adding one

The dashboard's Containers card is driven by **cAdvisor** scraped under the Prometheus
`job="cadvisor"`. `fetchContainers()` reads that job directly and groups by host; the
frontend renders a section per host when more than one host reports (no `CADVISOR_HOSTS`
env var — that belonged to an abandoned reimplementation and is unused).

Currently deployed cAdvisor hosts (image `gcr.io/cadvisor/cadvisor:v0.49.1`, standalone
`docker compose`, compose files in `deploy/<host>/docker-compose.cadvisor.yml`):
...
To add a cAdvisor host:
1. Deploy on the host: `docker compose -f docker-compose.cadvisor.yml up -d` (pick a free
   host port if 8080 is in use; map it to the container's 8080).
2. Add a target to the `cadvisor` job in `~/deploy/prometheus/prometheus.yml` on union:
   ```yaml
   - targets: ['<IP>:<port>']
     labels: { instance: <hostname> }
   ```
3. Reload Prometheus: `docker exec prometheus kill -HUP 1`. No dashboard restart needed —
   it reads the job live.
...
### Game servers (AMP) not showing / empty list

Symptom: the mobile app (and dashboard AMP card) shows no game servers, while
everything else works. `/api/amp/instances` returns `[]` with **HTTP 200** (no error).

Root cause: the dashboard caches its AMP session (`ampState.session`) process-wide.
When that session expires, **AMP replies with HTTP 200 and an `{"Title":"Unauthorized
Access", ...}` body — NOT a 401.** Older code only re-logged-in on `res.status === 401`,
so the stale session was never refreshed; the unauthorized object failed the
`Array.isArray()` check in the handler and silently became `[]`. This surfaces after the
container has been up long enough for the session to lapse (e.g. several days).

Fixed in `ampPost()` (2026-06-17): it now detects AMP's 200-with-auth-error body via
`isAmpAuthError()`, clears the cached session, and retries with a fresh login — so it
self-heals. The same path also covers AMP Start/Stop/Restart actions.

Diagnose / confirm AMP is reachable and what it returns (creds are in the container env):
```bash
# from union — query through the dashboard container (host can't reach the 18_Net IP)
docker exec nola-dashboard wget -qO- http://127.0.0.1:3000/api/amp/instances
# raw AMP check: a valid session lists instances; a stale one returns
#   {"Title":"Unauthorized Access", ...} with HTTP 200
```
Note: `ADSModule/GetInstances` returns an array whose `AvailableInstances` includes the
`ADS` controller itself — the handler filters `Module === 'ADS'` out, leaving only the
real game-server instances.

### Ask NOLA returning stale/wrong data

The `/api/ask` endpoint fetches live data on every request — it is not cached.
If the LLM answer seems wrong, check the raw context (query via the container, since the
union host cannot reach the macvlan IP directly):
```bash
docker exec nola-dashboard wget -qO- http://127.0.0.1:3000/api/data | python3 -m json.tool | head -60
```
...
Secret stored in: `~/nola/dashboard/.env` on union as `PVE_API_TOKEN`
...
| Service | URL | Notes |
|---------|-----|-------|
| **NOLA Dashboard** | **https://nola.example.com** | **This dashboard** |
| n8n | http://n8n.example.com | Workflow automation |
| Grafana | https://grafana.example.com | Dashboards & alerts |
| LibreNMS | https://librenms.example.com | Network monitoring |
| Authentik | https://auth.example.com | SSO / identity |
| Portainer (containy) | https://containy.example.com | Container management |
