...
export default function ChatScreen() {
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      if (settings.dashboardToken) headers['Authorization'] = `Bearer ${settings.dashboardToken}`;

      const res = await fetch(`${settings.dashboardUrl}/api/ask`, {
        method: 'POST',
        headers,
      ...
      const reply = data?.response ?? data?.error ?? JSON.stringify(data);
  ...
  }, [input, sending, messages, settings.dashboardUrl, settings.dashboardToken, scrollToEnd]);
  ...
  const renderItem = useCallback(({ item }) => {
    ...
    return (
      <View style={[styles.msgRow, isUser ? styles.msgRowUser : styles.msgRowAssistant]}>
        ...
        <View style={[styles.bubble, isUser ? styles.bubbleUser : isError ? styles.bubbleError : styles.bubbleAssistant]}>
          <MarkdownText text={item.text} style={[styles.bubbleText, isError && styles.bubbleTextError]} />
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={clearHistory} style={styles.clearBtn} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={0}>
        ...
        {sending && (
          <View style={styles.typingRow}>
            ...
            <View style={styles.typingBubble}><ActivityIndicator size="small" color="#7b7bff" /></View>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  mdCode: {
    ...
    borderRadius: 4, paddingHorizontal: 3, fontSize: 13,
  },
