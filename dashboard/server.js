...
const LINUX_HOSTS = (process.env.LINUX_HOSTS || 'containy,voyager,knox,dilithium,amp-server,union').split(',').map(h => h.trim());
...
const PVE_API_URL   = process.env.PVE_API_URL   || 'https://10.0.16.32:8006';
...
// ─── LLM helpers ────────────────────────────────────────────────────────────
function buildLabContext(prom, alerts, pve) {
  const lines = [];
...
app.post('/api/ask', express.json(), async (req, res) => {
  ...
  const [promResult, alertsResult, pveResult] = await Promise.allSettled([
    fetchPrometheus(),
    fetchAlerts(),
    fetchProxmox(),
  ]);
  ...
  const systemPrompt =
    ...
    `Current lab state:\n${buildLabContext(prom, alerts, pve)}`;
